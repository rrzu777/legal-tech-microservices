...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --backup-dir)
      ...
      ;;
    --operation-id)
      [ "$#" -ge 2 ] && [ -z "$requested_operation" ] || usage
...
wm_init || exit $?
[ "$wm_test" = "$test_mode" ] || exit "$EXIT_USAGE"
...
cleanup() {
  ...
  if [ -n "$resource_lock_fd" ]; then
    # No explicit unlock: delegated children share this open description.
...
do_rollback() { # backup-dir worker-restore-capability automatic-compensation
  ...
  local restore_worker_file='' restore_dropin_file='' manifest_path manifest_exists manifest_rel ignored
  while IFS=$'\t' read -r manifest_path manifest_exists manifest_rel ignored; do
    if [ "$manifest_path" = "$systemd_dir/estrado-pjud-worker.service" ]; then
      [ "$manifest_exists" = 1 ] || return 1
      restore_worker_file="$backup_dir/$manifest_rel"
    elif [ "$manifest_path" = "$systemd_dir/estrado-pjud-worker.service.d/xvfb.conf" ]; then
      [ "$manifest_exists" = 1 ] || return 1
      restore_dropin_file="$backup_dir/$manifest_rel"
    fi
  done < "$backup_dir/manifest.tsv"
  [ -n "$restore_worker_file" ] && [ -n "$restore_dropin_file" ] || return 1
  wm_cli status --unit-file "$restore_worker_file" --dropin-file "$restore_dropin_file" >"$null_file" || return 1
  # Durable protocol state is deliberately outside the restoration manifest.
  # A manual rollback from open must close/drain before restoring even one file.
  if [ "$automatic_compensation" -eq 0 ]; then
    local protocol_status protocol_state protocol_operation protocol_identity extra
    protocol_status=$(wm_cli status) || return 1
    read -r protocol_state protocol_operation protocol_identity extra <<< "$protocol_status"
    [ -z "${extra:-}" ] || return 1
    if [ "$protocol_state" = hold ]; then
  ...
  if [ "${desired_active_states[1]}" = active ] \
    ...
    fail 'rollback cannot authenticate a drained worker; no files restored'
    return 1
  fi
  ...
  # Rollback recovery is not permission to resume traffic.
  if [ "${desired_active_states[1]}" = active ]; then
    if [ "$wm_needs_restart_verify" = 1 ]; then wm_verify_restarted || return 1
    else wm_verify_current || return 1; fi
  fi
  check_public_health || return 1
...
run_apply_steps() {
  ...
  wm_cli status --unit-file "$worker_source" --dropin-file "$worker_dropin_source" >"$null_file" || return 1
  # Reject an unrestorable original before backup/hold/any lifecycle mutation.
  wm_cli status --unit-file "$worker_path" --dropin-file "$worker_dropin_path" >"$null_file" \
    || { fail 'current worker unit cannot be restored safely'; return 1; }
  ...
  PROV_ENABLE_PJUD_WORKER=0 PROV_SKIP_CADDY=1 wm_delegate "$provision_bin" >"$null_file" 2>&1 || provision_rc=1
...
run_apply() {
  ...
  durable_replace_metadata "$backup_dir/apply-result" $'succeeded\n' || return 1
  # Publishing open is outside automatic rollback: uncertain finalization must
  # not mutate a host on which queued work may already have resumed.
  wm_finish || return $?
...
case "$command_name" in
  ...
  finish)
    ...
    read -r protocol_state protocol_operation wm_identity extra <<< "$protocol_status"
    [ -z "${extra:-}" ] && [ "$protocol_operation" = "$requested_operation" ] || exit 1
    wm_operation_id=$requested_operation
    if [ "$protocol_state" = hold ]; then
      wm_wait || exit 1
      run_postflight || exit 1
      wm_finish
    elif [ "$protocol_state" = open ]; then
      wm_cli finish --operation-id "$wm_operation_id" --identity "$wm_identity" --global-fd "$wm_global_fd"
    else exit 1; fi
    ;;
esac
