...
# count exacto vía HEAD (read-only). Devuelve el total del content-range o "?"
cnt() {
  local cr
...
SYNC_ERR=$(cnt "cases?select=id&last_sync_status=eq.error")
SYNC_BLOCKED=$(cnt "cases?select=id&sync_blocked_until=gte.$NOW")
RUNS_24=$(cnt "case_sync_runs?select=id&created_at=gte.$SINCE")
RUNS_FAIL_24=$(cnt "case_sync_runs?select=id&created_at=gte.$SINCE&status=eq.error")
...
Sync 24h: $RUNS_24 corridas ($RUNS_FAIL_24 fallidas) | causas con error: $SYNC_ERR | bloqueadas ahora: $SYNC_BLOCKED
...
NO inventes números; usá solo estos. Terminá con "⚠️ Atención:" solo si hay algo que requiera acción.
...
if [ -z "${DIGEST// }" ]; then
  DIGEST="📊 Digest Estrado — $(date -u +%F)
