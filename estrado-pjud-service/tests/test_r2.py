...
@pytest.mark.asyncio
async def test_upload_stores_document():
    with patch("app.r2.boto3") as mock_boto:
        mock_s3 = MagicMock()
        mock_boto.client.return_value = mock_s3

        client = R2Client("key", "secret", "https://r2.example.com", "test-bucket")
        result = await client.upload("docs/test.pdf", b"%PDF-content", "application/pdf")
...
async def test_upload_rejects_oversized_document():
    with patch("app.r2.boto3") as mock_boto:
        ...
        with pytest.raises(ValueError, match="too large"):
            await client.upload("big.pdf", b"x" * (MAX_DOC_SIZE + 1), "application/pdf")
...
async def test_exists_returns_true_when_found():
    with patch("app.r2.boto3") as mock_boto:
        ...
        assert await client.exists("docs/test.pdf") is True
